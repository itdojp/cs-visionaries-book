...
const EXPECTED_REQUIRED = {
  ...
  appendixD: [
    ...
    '|Algorithm|モデルと学習法（深層学習、[PageRank]({{ glossary_pagerank_url }})、[Transformer]({{ glossary_transformer_url }})、最適化）|性能、汎化、推論、解釈可能性|'
  ]
};
...
const EXPECTED_FORBIDDEN = [
  ...
  '|Data|学習・評価・運用に必要なデータ基盤（[ImageNet]({{ glossary_imagenet_url }})、DB、Web、検索/[PageRank]({{ glossary_pagerank_url }})、ログ）'
];
