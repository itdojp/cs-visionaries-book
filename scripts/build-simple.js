...
`;
    ...
    content = this.insertGeneratedNotice(content, srcPath);
    
    await fs.writeFile(destPath, content, 'utf-8');
  }

  insertGeneratedNotice(content, srcPath) {
    const sourcePath = path.relative(process.cwd(), srcPath).replace(/\\/g, '/');
    const newline = content.includes('\r\n') ? '\r\n' : '\n';
    const noticePrefix = '<!-- generated by scripts/build-simple.js |';
    const notice = `<!-- generated by scripts/build-simple.js | authoring source: ${sourcePath} | do not edit docs/ directly | navigation canonical: docs/index.md | reader-facing preview: GitHub Pages -->${newline}${newline}`;
    const frontMatterMatch = content.match(/^---\r?\n[\s\S]*?\r?\n---(?:\r?\n)*/);

    if (!frontMatterMatch) {
      if (content.startsWith(noticePrefix)) {
        return content;
      }
      return notice + content;
    }

    if (content.slice(frontMatterMatch[0].length).startsWith(noticePrefix)) {
      return content;
    }

    return frontMatterMatch[0] + notice + content.slice(frontMatterMatch[0].length);
  }
