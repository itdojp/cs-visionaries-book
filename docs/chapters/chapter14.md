...
イベントAがイベントBの「前に起こった」と言えるのは、以下の場合のみです。
...
Byzantine Agreement が可能な必要十分条件は次のとおりです。
**n > 3f**
...
1980年代後半、ランポートは理論的な成果を実用的なシステムに応用する方法を模索していた。Byzantine Agreement は数学的に美しいが、実際のシステムでは次の問題があった。
