...
<!-- generated by scripts/build-simple.js | authoring source: src/chapters/chapter07.md | do not edit docs/ directly | navigation canonical: docs/index.md | reader-facing preview: GitHub Pages -->
...
1990年代後半、スタンフォード大学のコンピュータサイエンス棟。ここからは、公開された技術報告に至る研究上の問題意識を理解するための再構成である。深夜のコンピュータラボで、二人の大学院生が激しい議論を交わしていた。一人は金髪のラリー・ペイジ、もう一人は黒髪のセルゲイ・ブリン。テーブルの上には、数百枚のプリントアウトされたWebページが散乱していた。
...
この場面は、後に公開された「[PageRank]({{ '/appendices/appendix-b/#glossary-pagerank' | relative_url }})」の問題設定を物語として表現したものである。1990年代末、二人が関わった研究は、link構造を利用する検索engine「Google」として公表された。

あなたが何かを知りたいとき、反射的に検索boxへ言葉を打ち込む。初期PageRankは、Webのlink構造からpageの重要度を評価する発想を示した。ただし、現在の検索serviceは複数のranking signal、indexing、自然言語処理などを組み合わせるため、初期PageRankだけで検索結果を説明することはできない。
...
1995年当時、World Wide Webは急速に拡大していた。ティム・バーナーズ＝リーが1991年に公開してから僅か4年で、数千万のWebページが存在していた。しかし、この膨大な情報の中から必要な情報を見つけることは、非常に困難だった。
...
**[現代との接続：PageRankと後続技術の関係]**

PageRankは1990年代末の技術報告で説明されたlink解析algorithmである。graph上で重要度を反復計算する考え方は、後続のgraph解析を理解する一例になる。ただし、PageRankをすべての現代AIの直接的な基盤とみなすことはできない。

**共通するgraph上の問題設定**：

- **graph ranking**：nodeとedgeから相対的重要度を計算する
- **推薦system**：userとitemの関係をgraphで表現する場合がある
- **social network分析**：接続関係や影響の伝播を扱う
- **知識graph**：概念間の関係を構造化する

これらはgraphを扱うという共通点を持つが、個々の手法がPageRankを直接利用するとは限らない。また、BERT、GPT、Transformerは別の研究系譜に属する。歴史的な関連、構造上の類似、直接的な技術継承を区別して読む必要がある。

検索serviceのranking signalやmodel構成は更新され続ける。製品固有の構成を説明するときは、公式資料の版と確認日を併記する。

検索技術は、link解析、情報検索、自然言語処理など複数の研究と実装を取り込みながら更新されている。ラリーとセルゲイが示した検索engineの構想と後年の製品を比較することはできるが、両者を単一のalgorithmによる直線的な進化とはみなさない。製品の現行機能を論じる場合は、確認日付きの公式資料が必要である。
...
**現代への応用**:
- **AI / ML 投資**：機械学習、深層学習、自然言語処理技術の活用
