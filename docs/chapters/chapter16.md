...
## 16.5 生成モデルとTransformer（2014〜2017）

深層学習は「認識（分類）」だけでなく「生成」へ拡張され、2014年のGANと2017年のTransformerで大きな転換点を迎えた。本章では **2017年まで**（深層学習史の骨格）を扱い、**2018年以降の大規模言語モデル（LLM）と生成AIの社会実装**は、第10章で扱う。
...
Transformerは、その後のBERT/GPT等の言語モデルへ接続し、2020年代の生成AIの社会実装を加速させた。ここから先（LLM、マルチモーダル、ツール統合、評価とガードレール、規制対応など）は **第10章** と **付録A（年表）** を参照してほしい。

注記:
- モデルの詳細（例: サイズ、学習データの全容）は、競争上・安全上の理由で **非公開** とされている場合がある。本書ではそのような情報については「非公開」と表記し、公表されている推定値を紹介する際には出典を明記したうえで「推定（出典あり）」と表記する。
...
ジェフリー・ヒントンの物語は、科学的信念の力と長期視点の重要性を示している。AI冬の時代に多くの研究者が諦める中、彼は「脳の仕組みを理解し再現する」という信念を貫いた。その結果、現代社会を変革する深層学習革命を実現した。基礎研究の積み重ねが、後年の巨大な産業と社会実装へつながることを示す事例でもある。
