...
<!-- generated by scripts/build-simple.js | authoring source: src/chapters/chapter10.md | do not edit docs/ directly | navigation canonical: docs/index.md | reader-facing preview: GitHub Pages -->
...
2012年10月、トロント大学の研究室。ジェフリー・ヒントンは、コンピュータの画面に映る結果を信じられない思いで見つめていた。[ImageNet]({{ '/appendices/appendix-b/#glossary-imagenet' | relative_url }})画像認識コンテストの最終結果が発表された瞬間だった。
...
<figure class="book-figure">
  <img src="{{ '/assets/images/diagrams/hinton-ai-history-timeline.svg' | relative_url }}" alt="1947年の誕生から2023年のGoogle退職まで、ジェフリー・ヒントンとAI発展の節目を並べた年表">
  <figcaption>図10-1　ジェフリー・ヒントンとAI発展の歴史</figcaption>
</figure>
...
| --- | --- |
| 1947年 | イギリスに生まれる |
| 1970年 | ケンブリッジ大学で実験心理学を専攻し、認知科学への関心を深める |
| 1978年 | エディンバラ大学でAIの博士号を取得し、人工知能研究へ進む |
| 1986年 | ラメルハート、ウィリアムズとの共同研究でバックプロパゲーション法を発表する |
| 1987〜2001年 | 研究資金削減と人材流出が進むAI冬の時代にも研究を継続する |
| 2006年 | 深層信念ネットワークを発表し、AI研究復活の兆しを示す |
| 2012年 | トロント大学の研究チームによるAlexNetがImageNetで優勝し、深層学習の実用性を示す |
| 2013年 | GoogleがDNNresearchを買収し、ヒントンがGoogleに加わる |
| 2018年 | ヤン・ルカン、ヨシュア・ベンジオとともにチューリング賞を受賞する |
| 2023年 | Googleを退職し、AIの安全性に警鐘を鳴らす |
...
- **[GPU]({{ '/appendices/appendix-b/#glossary-gpu' | relative_url }})並列計算**：NVIDIA GeForce GTX 580による高速化
...
│      └─ 残差接続による超深層化
│
2017年: Attention Is All You Need発表
│      └─ 注意機構による言語処理革命
│
2020年: GPT-3
│      └─ 大規模言語モデルの衝撃
...
### [Transformerアーキテクチャ]({{ '/appendices/appendix-b/#glossary-transformer' | relative_url }})の登場
...
2022年のChatGPT以降、生成AIは「技術デモ」から「業務で使う道具」へ移行しつつある。変化点はモデル名の羅列ではなく、**機能の潮流**として捉えると実務に接続しやすい（年表と一次情報は付録A/付録C参照）。

**マルチモーダル**：
音声/画像/テキスト等を同一の対話で扱い、入力や状況の理解が広がる。一方で、個人情報・機密情報の混入経路も増えるため、ログ/スクショ/録音の取り扱いとマスキングが重要になる。

**推論（deliberate reasoning）**：
即答ではなく、思考時間を使って段階的に推論するタイプのモデルが注目される。実務では「どの条件なら推論を使うべきか」「誤答時の検出（Evals）」「人間の承認点」を設計する必要がある。

**長コンテキスト**：
長い文脈を前提に、仕様書・設計・ログなど“長い一次情報”を扱いやすくなる。一方で、投入するデータの権利・秘匿・保持期間（ログ保管を含む）を定義しないと事故につながる。

**ツール呼び出し/エージェント化**：
検索、DB問い合わせ、コード実行、チケット更新などのツールを組み合わせ、計画→実行→検証のループでタスクを進める形態が増える。成功には、権限分離（最小権限）、監査ログ、ロールバック、誤操作時の隔離が不可欠である。

**評価（Evals）・ガードレール・ガバナンス**：
「便利」だけでは業務で定着しない。品質（正確性/再現性）と安全（漏えい/誤誘導）を、テストと運用で守る必要がある。生成AIの導入は、従来のソフトウェアよりも“変更が壊しやすい”ため、評価（回帰）とガードレールを運用に組み込む。

**規制と企業対応（EU AI Act 等）**：
規制は“技術の話”ではなく“責任分界と運用の話”である。企業は、用途分類（リスク分類）、説明責任、記録（証跡）、人間の監督を前提に、導入範囲と手順を設計する必要がある。
...
<figure class="book-figure">
  <img src="{{ '/assets/images/diagrams/neural-network-layer-flow.svg' | relative_url }}" alt="3つの入力、3つと2つのニューロンからなる2つの隠れ層、2つの出力を隣接層ごとに全結合したネットワーク">
  <figcaption>図10-2　入力層から出力層までのニューラルネットワーク</figcaption>
</figure>

### 図の代替説明：ニューラルネットワークの接続

1. 入力層は入力1、入力2、入力3の3要素を受け取る。
2. 各入力は、隠れ層1のニューロン1〜3のすべてに接続する。
3. 隠れ層1の各ニューロンは、隠れ層2のニューロン1〜2のすべてに接続する。
4. 隠れ層2の各ニューロンは、出力1と出力2の両方に接続する。

この例は、隣り合う層の各要素を相互に結ぶ全結合ネットワークであり、情報は入力層から2つの隠れ層を経て出力層へ伝わる。
...
| **2000年代** | サポートベクター<br>マシン、決定木 | マルチコアCPU | GB〜TB | スパムフィルタ、推薦システム |
| **2010年代** | 深層学習 | GPU並列計算 | TB〜PB | 画像認識、自然言語処理 |
| **2020年代** | 大規模言語モデル | 分散GPU<br>クラスタ | PB〜EB | ChatGPT、GPT-4 |
...
<figure class="book-figure">
  <img src="{{ '/assets/images/diagrams/traditional-ai-vs-deep-learning.svg' | relative_url }}" alt="人手で作成したルールや特徴を使う従来AIと、生データから特徴を自動抽出する深層学習AIの処理の比較">
  <figcaption>図10-3　従来AIと深層学習AIの処理比較</figcaption>
</figure>

### 図の代替説明：従来AIと深層学習AIの違い

| 比較対象 | 入力・人間の役割 | 処理の流れ |
| --- | --- | --- |
| ルールベースの従来AI | 人間がルールを作成する | 作成したルールを論理推論エンジンへ渡す |
| 機械学習を使う従来AI | 人間が特徴を設計する | 設計した特徴を機械学習アルゴリズムへ渡す |
| 深層学習AI | 大量の生データを用意する | 深層ニューラルネットワークが特徴を自動抽出し、その表現を使ってタスクを解く |
