...
<!-- generated by scripts/build-simple.js | authoring source: src/chapters/chapter03.md | do not edit docs/ directly | navigation canonical: docs/index.md | reader-facing preview: GitHub Pages -->
...
1947年9月9日、午後3時45分。ハーバード大学計算実験室で、Harvard Mark II のログに「First actual case of bug being found」と記された、有名な故障記録が残された。

巨大なコンピュータ「Harvard Mark II」が突然動作を停止した。室内には機械の冷却音だけが響いている。技術者たちが原因を探る中、リレー接点に挟まった小さな蛾が見つかった。

技術者たちは蛾を取り除き、それをテープで日誌に貼り付けて「First actual case of bug being found（初の実際のバグ発見例）」と記録した[^1]。この逸話はホッパーの現場経験を象徴するものとして語り継がれている。

「bug」という語は以前から工学分野で使われていたが、この記録はコンピュータ史で象徴的な逸話として知られている。ホッパーのより大きな貢献は、機械語に閉じていた計算機を、人間が扱いやすい言語へ近づけようとした点にあった。

「なぜコンピュータは、0と1しか理解できないのか？人間が話す言葉で命令できたらどんなに素晴らしいだろう」—グレースは考えた。そして彼女は、この夢を現実に変えた。

あなたが今使っているプログラム、スマートフォンのアプリ、ウェブサイト。これらを支える高水準言語とソフトウェア標準化の流れには、1950年代にホッパーらが進めた「人間が扱いやすい言語」という発想が重要な源流の一つとして残っている。
...
1906年12月9日、グレース・ブリュースター・マレーは、ニューヨーク市で生まれた。父ウォルター・フレッチャー・マレーは保険会社の上級役員、母メアリー・キャンベル・ヴァン・ホーンは数学と語学に造詣の深い女性だった[^2]。

グレースの数学的才能は、幼い頃から際立っていた。7歳の時、彼女は目覚まし時計7個を分解して、その仕組みを理解しようとした。「なぜこの小さな機械が正確に時を刻むのか知りたかった」と、後に彼女は回想している[^3]。
...
1934年、グレースはイェール大学で数学の博士号を取得した。博士論文のテーマは「既約代数多様式の新分類」という高度に抽象的な内容だった[^4]。しかし、この理論的基盤が、後にコンピュータプログラムの構造を理解する上で重要な役割を果たすことになる。

博士号取得後、グレースはヴァッサー大学の数学科で教鞭をとった。彼女は学生たちに、数学の美しさと実用性の両方を教えようとした。「数学は抽象的な概念ですが、現実世界の問題を解決する最も強力な道具でもあります」—これが彼女の教育哲学だった。

### 第二次大戦、海軍への志願

1941年12月7日、真珠湾攻撃によりアメリカが第二次世界大戦に参戦した。グレースの私生活にも大きな変化が起こった。1945年、夫ヴィンセントとの離婚が成立し、彼女は再び独身となった[^5]。
...
1959年、FLOW-MATIC や商用データ処理言語の経験を背景に、国防総省を含む政府・企業関係者は CODASYL を通じて共通ビジネス用言語の検討を進めた。ホッパーは FLOW-MATIC と「英語に近いプログラミング」への主張を通じて COBOL の方向性に強い影響を与えたが、単独のプロジェクトリーダーとしてではなく、委員会方式の標準化と産業界の合意形成の中で貢献した。

### プログラミングの民主化

1959年、新しいプログラミング言語「COBOL（Common Business Oriented Language）」の初期仕様がまとまった。COBOL は、ホッパーが FLOW-MATIC で追求した「英語に近い業務記述」を、複数組織の標準化プロセスで共有可能な形へ押し広げた言語だった。
...
この変化は、コンピュータを一部の専門家だけの機械から、業務担当者が関与できる道具へ近づける流れとして位置づけられる。
...
自然言語とプログラミングの距離を縮める流れは、AI技術の発展により新しい段階に入っている。ChatGPTなどの大規模言語モデルは自然言語の指示からプログラムコードを生成できるが、これは COBOL の直線的な延長ではなく、「人間が意図を伝えやすい抽象化」を追求する別系統の技術発展として捉える必要がある。
...
各社独自の言語が増えれば互換性と移植性が失われる、という問題意識が標準化の背景にあった。
...
1959年に初期仕様がまとまった後、COBOL は実装、検証、標準化という段階へ進んだ。国防総省、CODASYL、メーカー、ユーザー組織が関与し、実務で使える共通言語にするためには、厳密な仕様定義と品質保証が必要だった。

ホッパーは標準化の重要性を訴え、FLOW-MATIC で得た知見を共有する立場から、以下の課題に関わった：
...
COBOL の成功により、標準化は国防総省、CODASYL、メーカー、ユーザー組織から、のちの ANSI / ISO など複数組織へ広がった。ホッパーは国際的なプログラミング言語標準化の重要性を訴えた人物の一人だが、標準化そのものは複数の委員会と産業界の合意形成によって進んだ。

**国防総省 / CODASYL 周辺の活動**：
- 共通ビジネス言語の必要性を整理
- FLOW-MATIC の知見を COBOL の英語風構文へ接続
- 異機種間で利用できる言語仕様の重要性を明確化
...
この標準化への関心は、現代のソフトウェア業界で重視される相互運用性や公開仕様の考え方と接続できる。ただし、Web標準や通信プロトコルは別の歴史的文脈で発展したため、ホッパー個人の成果としてではなく、複数世代にわたる標準化文化の一部として理解する必要がある。
...
### 学術論文
1. Sammet, Jean E. (1978). "The Early History of COBOL". ACM SIGPLAN Notices, 13(8), pp.121-161. https://doi.org/10.1145/960118.808378
