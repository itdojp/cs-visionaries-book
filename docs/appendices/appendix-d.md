...
<!-- generated by scripts/build-simple.js | authoring source: src/appendices/appendix-d.md | do not edit docs/ directly | navigation canonical: docs/index.md | reader-facing preview: GitHub Pages -->
...
現行版の参照先メモ: [Web / インターネット（第6章）]({{ '/chapters/chapter06.html' | relative_url }})、[検索（第7章）]({{ '/chapters/chapter07.html' | relative_url }})、[SNS（第8章）]({{ '/chapters/chapter08.html' | relative_url }})、[クラウド（第9章）]({{ '/chapters/chapter09.html' | relative_url }})、[AI（第10章）]({{ '/chapters/chapter10.html' | relative_url }}) です。章番号で迷った場合は、トップページの目次を正としてください。
...
3. [第10章（現代AIの社会実装）]({{ '/chapters/chapter10.html' | relative_url }}) を読み、必要に応じて [付録C]({{ '/appendices/appendix-c.html' | relative_url }}) の AI 文献で技術史を補う
4. データ/運用/ガバナンスの章（[第6章]({{ '/chapters/chapter06.html' | relative_url }}) / [第7章]({{ '/chapters/chapter07.html' | relative_url }}) / [第2章]({{ '/chapters/chapter02.html' | relative_url }}) / [第9章]({{ '/chapters/chapter09.html' | relative_url }})）を必要に応じて補完する
...
詳細な年号、出来事の並び、補助的な節目を確認したい場合は、まず [付録A]({{ '/appendices/appendix-a.html' | relative_url }}) を参照してください。本付録では、AI 時代形成を理解するための代表イベントだけを残しています。
